 ...
 *
 */
void build_ethernet_header(struct ox_packet_struct * recv_ox_p, struct ox_packet_struct * send_ox_p) {
 ...
 *
 */
void build_tLoE_frame_header(int connection_id, struct ox_packet_struct * recv_ox_p, struct ox_packet_struct * send_ox_p) {
 ...
 * @brief
 */
int send_ack(int sockfd, int connection_id, struct ox_packet_struct * recv_ox_p)
{
 ...
 * @brief
 */
int send_close_connection(int sockfd, int connection_id, struct ox_packet_struct * recv_ox_p)
{
	char send_buffer[RECV_BUFFER_SIZE] = {0, };
	int send_buffer_size = 0;
	struct ox_packet_struct send_ox_p;
 ...
 */
int ox_struct_to_packet(struct ox_packet_struct * ox_p, char * send_buffer, int * send_buffer_size) {
	int packet_size = 0;
	uint64_t mask;
	uint64_t be64_temp;
	int offset = 0;
	...
	if ( ox_p->flit_cnt < 5 ) { //at minimum. packet_size must be 70
		packet_size += (sizeof(struct eth_header) + sizeof(struct tloe_header) + \
				(sizeof(uint64_t) * 5) + sizeof(ox_p->tl_msg_mask));
	} else {
		packet_size += (sizeof(struct eth_header) + sizeof(struct tloe_header) + \
				(sizeof(uint64_t) * ox_p->flit_cnt) + sizeof(ox_p->tl_msg_mask));
	}
	...
	if ( ox_p->flit_cnt > 0  && ox_p->flits != NULL ) {
		memcpy(send_buffer+offset, ox_p->flits, sizeof(uint64_t) * ox_p->flit_cnt);
		PRINT_LINE("ox_p->flits[0]=0x%lx\n", ox_p->flits[0]);
	}

	// TLoE frame mask
	mask = htobe64(ox_p->tl_msg_mask);
	memcpy(send_buffer+packet_size-sizeof(uint64_t), &mask, sizeof(uint64_t));
 ...
 */
int packet_to_ox_struct(char * recv_buffer, int recv_size, struct ox_packet_struct * ox_p)
{
	...
	*(uint64_t*)&__tl_msg_hdr = be64toh(temp_tl_msg_hdr);
	...
	memcpy(&(ox_p->eth_hdr), recv_packet_eth_hdr, sizeof(struct eth_header));

	// TLoE frame header (8 bytes)
	recv_packet_tloe_hdr = (struct tloe_header *)(recv_buffer + sizeof(struct eth_header));
